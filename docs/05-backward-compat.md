...
> **Goal**: Preserve existing metadata shapes while migrating to ATS standard. No breaking changes.
...
**Challenge**: We cannot break existing callers. All new ATS fields must be **optional** and **additive**.

---

## Solution 1: TaskCreate/TaskUpdate Collision — Nested `_dgm` Key

`TaskCreate` and `TaskUpdate` already use `metadata` as an **arbitrary user data object** for tracking task-specific context. ATS fields **MUST nest under `_dgm` key** to avoid collision:
...
**Critical constraint**: Task tools preserve user's arbitrary `metadata` keys at the top level. ATS metadata lives **exclusively** under `metadata._dgm`.
...
**Purpose**: Distinguish "old caller hasn't adopted ATS yet" from "Tier 3 tool intentionally uses minimal metadata".

| Scenario | `is_legacy_compat` | Interpretation |
|----------|-------------------|----------------|
| Pre-ATS caller (no `_dgm` block) | `true` | Old caller — not yet migrated |
| Tier 3 tool (only `source` + `provenance`) | `false` | Intentionally minimal — ATS-compliant |
| Tier 1/2 tool (full metadata) | `false` | Fully adopted ATS standard |
...
3. Emit full ATS metadata with all 5 goal fragments
...
2. Set `_compat.schema_version="1.0.0"`
3. **Task tools**: nest ATS fields under `metadata._dgm` key
...
def parse_metadata(raw_metadata: dict) -> Metadata:
    ...
    if version < "1.0.0":
        # Legacy pre-ATS format — apply migration shim
...
The existing telemetry schema at `~/.claude/configs/claude-code-schema/unified-activity.schema.json` validates **top-level event records**. ATS metadata appears **nested inside tool-specific fields**.
...
```json
{
  "properties": {
    ...
    "_dgm": {
      ...
      "description": "Optional ATS metadata for tools that emit it"
    }
  }
}
```

1. **Extract and validate** ATS metadata separately:

```python
# Validate top-level event structure
jsonschema.validate(event_record, unified_activity_schema)

# If tool emits ATS metadata, validate nested block
...
| Concern | Solution |
|---------|----------|
| TaskCreate/TaskUpdate collision | Nest ATS under `metadata._dgm` key |
...
**Migration order**: Tier 1 → Tier 2 → Tier 3, with each tier setting `schema_version="1.0.0"` to signal ATS compliance.
